...
export class ScraperProcessor extends WorkerHost {
  ...
  async process(
    ...
    this.logger.log(`Processing job for organization ${orgIdString}`);
    ...
    const org = await this.organizationService.getOrganizationFromId(orgId);

    const browser = new Browser(org.url, 1024, 768);

    try {
      await browser.launch();
    } catch (error) {
      const errorMessage = `Error launching browser for organization.`;
      this.logger.error(errorMessage + " " + orgId + " " + error);
    ...
    if (await BoardDocsScraper.checkWillScrape(browser.page)) {
      ...
      this.logger.log(`Using GarnetValleyScraper for organization ${orgId}`);
      ...
      this.logger.log(`Using RidleyScraper for organization ${orgId}`);
    }

    if (!scraper) {
      const errorMessage = `No scraper found for organization`;
      this.logger.error(errorMessage + " " + orgId);
      await this.organizationService.updateLastScrapedError(
        orgId,
        errorMessage,
      );
    ...
    this.logger.log(`Scraped data for organization ${orgId}`);
    ...
    try {
      ...
      await this.organizationService.updateLastScrapedDate(orgId, new Date());
    } catch (error) {
      let errorMessage = "Unknown error";
      if (error instanceof Error) {
        errorMessage = error.message;
